...
import { EntityNotFoundError, ForbiddenError } from '@dersim/shared';
import { IMessage, IDisplayMessage } from '@dersim/shared';
...
const SessionService = {
    ...
    async resetContextWindow(sessionId: string): Promise<ISession> {
        ...
        // Move all display messages to immediate
        const messages: IMessage[] = session.displayMessages.map(message => {
            const rawMessage: IMessage = {
                role: message.role,
                content: message.content,
            }
            return rawMessage;
